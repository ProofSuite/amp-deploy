if [ ! "$(docker ps -q -f name=rabbitmq)" ]; then
    ...
    docker cp ./rabbitmq-local.conf rabbitmq:/etc/rabbitmq/rabbitmq.conf
    docker cp ${AMP_CERTS_PATH}/rabbitmq-staging/ca_certificate.pem rabbitmq:/etc/ssl/ca_certificate.pem
    docker cp ${AMP_CERTS_PATH}/rabbitmq-staging/ca_key.pem rabbitmq:/etc/ssl/ca_key.pem
    docker cp ${AMP_CERTS_PATH}/rabbitmq-staging/server_key.pem rabbitmq:/etc/ssl/server_key.pem
    docker cp ${AMP_CERTS_PATH}/rabbitmq-staging/server_certificate.pem rabbitmq:/etc/ssl/server_certificate.pem
