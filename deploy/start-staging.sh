...
eval $(docker-machine env client)
CONFIG_VERSION=${STAGING_CONFIG_VERSION} docker stack deploy -c docker-compose.staging-frontend.yml amp-staging-frontend --prune

eval $(docker-machine env matching-engine)
CONFIG_VERSION=${STAGING_CONFIG_VERSION} docker stack deploy -c docker-compose.staging-backend.yml amp-staging-backend --prune